package com.ryan.videoplayer.decoder;
...
public abstract class MediaCodecDecoderLifeCycle {
	...
	public boolean isHWCodecAvaliableFromNative() {
		boolean ret = false;
		Log.i("zhf-debug", "useMediaCodec " + useMediaCodec);
		if (useMediaCodec) {
			String manufacturer = Build.MANUFACTURER;
			String model = Build.MODEL;
			Log.d("zhf-debug", "manufacturer=" + manufacturer + " model=" + model);
	...
	public boolean createVideoDecoderFromNative(int width, int height, int texId, byte[] sps, int spsSize, byte[] pps,
			int ppsSize) {
		Log.e("zhf-debug", "createVideoDecoderFromNative 22222222");
